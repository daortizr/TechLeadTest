...
export class PaymentRepositoryImpl implements PaymentRepository {
  async insertAuthorized(
    ...
    await manager.query(
      `
      INSERT INTO payments (idempotency_key, authorization_ref, amount_cents, status)
      VALUES ($1, $2, $3, 'AUTHORIZED')
      `,
      [idempotencyKey, authorizationRef, amountCents]
    );
  }

  async insertDeclined(tx: TransactionContext, idempotencyKey: string, amountCents: number): Promise<void> {
    const manager = UnitOfWorkImpl.getManager(tx);
    await manager.query(
      `
      INSERT INTO payments (idempotency_key, amount_cents, status)
      VALUES ($1, $2, 'DECLINED')
      `,
  ...
  async findAuthorizedWithoutReservation(tx: TransactionContext, idempotencyKey: string): Promise<Payment | null> {
    ...
    const rows = returningRows<PaymentRecord>(
      await manager.query(
        `
        SELECT id, idempotency_key, reservation_id, authorization_ref, amount_cents, status, created_at
        FROM payments
