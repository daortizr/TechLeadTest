...
export const FLIGHT_CODES = ['AV101', 'AV102', 'AV103', 'AV104', 'AV105', 'AV106', 'AV107'];
...
// The demo data: airports, seven flights with their seats and some sold seats, all with dates
// relative to the database clock. Used by the seed migration and by the startup refresh.
export async function seedDemoData(queryRunner: QueryRunner): Promise<void> {
  ...
  const existingAirports = new Set((await airportRepo.find()).map((airport) => airport.code));
  await airportRepo.insert(airports.filter((airport) => !existingAirports.has(airport.code)));
...
// Removes the seed flights and everything that hangs from them; other flights are not touched
export async function removeSeedFlights(queryRunner: QueryRunner): Promise<void> {
  const flightRepo = queryRunner.manager.getRepository(FlightEntity);
  const seatRepo = queryRunner.manager.getRepository(SeatEntity);
  const reservationRepo = queryRunner.manager.getRepository(ReservationEntity);
  const idempotencyRepo = queryRunner.manager.getRepository(IdempotencyKeyEntity);
...
export async function removeSeedAirports(queryRunner: QueryRunner): Promise<void> {
  const airportRepo = queryRunner.manager.getRepository(AirportEntity);
